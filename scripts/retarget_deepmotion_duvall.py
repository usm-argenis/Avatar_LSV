...
# ====================================================================
# RETARGET DEEPMOTION -> DUVALL
# Convierte animaciones de Deepmotion a armadura de Duvall
# ====================================================================
...
# Avatar destino
DUVALL_GLB = BASE_DIR / "test" / "output" / "glb" / "Duvall" / "Duvall_TPose.glb"

# Mapeo de huesos Deepmotion -> Duvall
BONE_MAPPING = {
    # Torso
    'hips_JNT': 'Hips',
    'spine_JNT': 'Spine',
...
def retarget_animation(source_glb, output_path, palabra):
    """
    Retargeta animación de Deepmotion a Duvall
    ...
    """
    print(f"\n{'='*70}")
    print(f"RETARGETING: {source_glb.name} -> Duvall")
    ...
    # 2. Importar Duvall T-Pose
    print(f"\n📥 Cargando Duvall T-Pose...")
    bpy.ops.import_scene.gltf(filepath=str(DUVALL_GLB))
    ...
    if not target_armature:
        print("❌ No se encontró armature de Duvall")
        return False
    
    print(f"   ✅ Duvall armature: {target_armature.name}")
    print(f"   ✅ Meshes: {len(target_meshes)}")
    
    # 3. Crear nueva acción para Duvall
    ...
    # Seleccionar solo Duvall (armature + meshes)
...
def main():
    ...
    if len(argv) < 1:
        ...
        print("Uso: blender --background --python retarget_deepmotion_duvall.py -- <archivo> [palabra]")
        sys.exit(1)
    ...
    output_dir = BASE_DIR / "test" / "output" / "glb" / "Duvall" / "deepmotion"
