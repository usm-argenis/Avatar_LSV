...
output_path = Path(r"C:\Users\andre\OneDrive\Documentos\tesis\test\output\Duvall_abril_brazos_combinado_v7.blend")
...
print("\nCopiando animación de brazos - enfoque simplificado...")
...
for frame in range(frame_start, frame_end + 1):
    ...
    for fbx_bone_name, glb_bone_name in bone_mapping.items():
        ...
        # MÉTODO NUEVO: Usar bone basis (la rotación del hueso respecto a su rest pose)
        # El bone.matrix_local es el rest pose
        # El pose_bone.matrix es el pose actual
        
        # Para FBX: obtener la rotación relativa al rest pose
        # pose_matrix = rest_matrix @ rotation_offset
        # rotation_offset = rest_matrix.inverted() @ pose_matrix
        
        fbx_rest = fbx_bone.matrix_local
        fbx_pose = fbx_pose_bone.matrix
        
        # Si tiene padre, necesitamos la matriz en parent bone space
        if fbx_pose_bone.parent:
            fbx_parent_rest = fbx_pose_bone.parent.bone.matrix_local
            fbx_parent_pose = fbx_pose_bone.parent.matrix
            # La rotación del parent
            fbx_parent_rotation = fbx_parent_rest.inverted() @ fbx_parent_pose
            # Aplicar la rotación del parent al rest pose del hueso actual
            fbx_rest_with_parent = fbx_parent_rotation @ fbx_rest
        else:
            fbx_rest_with_parent = fbx_rest
        
        # Ahora obtener la rotación relativa
        fbx_rotation_offset = fbx_rest_with_parent.inverted() @ fbx_pose
        
        # Para GLB: aplicar la misma rotación relativa
        glb_rest = glb_bone.matrix_local
        
        if glb_pose_bone.parent:
            # En GLB, aplicar considerando el parent
            glb_target_matrix = glb_rest @ fbx_rotation_offset
            
            # Convertir a local space del parent
            glb_parent_matrix = glb_pose_bone.parent.matrix
            glb_local_matrix = glb_parent_matrix.inverted() @ glb_target_matrix
        else:
            glb_local_matrix = glb_rest @ fbx_rotation_offset
