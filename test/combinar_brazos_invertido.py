"""
SOLUCIÓN: Invertir rotaciones de brazos para corregir dirección opuesta
"""
...
print("="*80)
print("SOLUCIÓN: Brazos con rotaciones invertidas/reflejadas")
print("="*80)
...
output_path = Path(r"C:\Users\andre\OneDrive\Documentos\tesis\test\output\Duvall_abril_BRAZOS_INVERTIDO.glb")
...
print(f"\n🎬 Copiando animación con inversión de rotaciones...")
...
for frame in range(frame_start, frame_end + 1):
    ...
    for fbx_bone_name, glb_bone_name in ARM_BONES_MAPPING.items():
        ...
        # INVERTIR la rotación: Rotar 180° en el eje Y (hacia arriba)
        # Esto invierte la dirección de los brazos
        inversion = Quaternion((0, 1, 0), math.pi)  # 180° en Y
        fbx_rot_inverted = inversion @ fbx_rot_world
        
        # Convertir a rotación local para GLB
        glb_parent_mat = glb_armature.matrix_world @ glb_pb.parent.matrix if glb_pb.parent else glb_armature.matrix_world
        glb_rot_local = glb_parent_mat.inverted().to_quaternion() @ fbx_rot_inverted
...
print(f"🎬 Frames: {frame_start}-{frame_end}")
print(f"✨ Método: Rotaciones INVERTIDAS 180° en eje Y")
