...
output_path = Path(r"C:\Users\andre\OneDrive\Documentos\tesis\test\output\Duvall_abril_brazos_combinado_v6.blend")
...
# Calcular offsets de orientación entre rest poses
print("\nCalculando offsets de orientación...")
rotation_offsets = {}
...
for fbx_bone_name, glb_bone_name in bone_mapping.items():
    if fbx_bone_name in fbx_armature.data.bones and glb_bone_name in glb_armature.data.bones:
        ...
        # Obtener rotaciones de rest pose en armature space
        fbx_rest_rot = fbx_bone.matrix_local.to_quaternion()
        glb_rest_rot = glb_bone.matrix_local.to_quaternion()
        
        # Calcular el offset: qué rotación necesitamos aplicar para ir de FBX rest a GLB rest
        # glb_rest = offset @ fbx_rest
        # offset = glb_rest @ fbx_rest^-1
        offset = glb_rest_rot @ fbx_rest_rot.inverted()
        rotation_offsets[fbx_bone_name] = offset
        
        print(f"{fbx_bone_name} -> {glb_bone_name}: offset = {offset}")

print("\nCopiando animación de brazos con offset de orientación...")
...
for frame in range(frame_start, frame_end + 1):
    ...
    for fbx_bone_name, glb_bone_name in bone_mapping.items():
        ...
        # Obtener la rotación del FBX en armature space
        fbx_armature_rot = (fbx_armature.matrix_world @ fbx_pose_bone.matrix).to_quaternion()
        
        # Aplicar el offset de orientación
        offset_rot = rotation_offsets[fbx_bone_name]
        adjusted_rot = offset_rot @ fbx_armature_rot
        ...
        if glb_pose_bone.parent:
            ...
            parent_rot = parent_world_matrix.to_quaternion()
            local_rot = parent_rot.inverted() @ adjusted_rot
        else:
            armature_rot = glb_armature.matrix_world.to_quaternion()
            local_rot = armature_rot.inverted() @ adjusted_rot
