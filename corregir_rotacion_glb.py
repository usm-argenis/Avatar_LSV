#!/usr/bin/env python3
"""
Script para corregir la rotación de archivos GLB volteados
...
def procesar_carpeta(carpeta, patron="*.glb", hacer_backup=True):
    ...
    archivos = list(carpeta.rglob(patron))
...
if __name__ == "__main__":
    ...
    # Opción 1: Procesar archivo individual
    archivo_individual = Path(r"C:\Users\andre\OneDrive\Documentos\tesis\test\output\glb\Duvall\verbos\Duvall_resultado_agarrar.glb")
    
    # Opción 2: Procesar toda la carpeta de verbos
    carpeta_verbos = Path(r"C:\Users\andre\OneDrive\Documentos\tesis\test\output\glb\Duvall\verbos")
    
    # Menú
    print("""
╔═══════════════════════════════════════════════════════════════╗
║     CORRECTOR DE ROTACIÓN GLB - Duvall Animations            ║
╚═══════════════════════════════════════════════════════════════╝

Selecciona una opción:
1. Corregir solo 'agarrar.glb'
2. Corregir TODOS los archivos en carpeta 'verbos'
3. Salir
""")
    ...
    if opcion == "1":
        if archivo_individual.exists():
            procesar_archivo(archivo_individual, hacer_backup=True)
        else:
            print(f"❌ Archivo no encontrado: {archivo_individual}")
    
    elif opcion == "2":
        if carpeta_verbos.exists():
            confirmar = input(f"⚠️ Se procesarán TODOS los GLB en {carpeta_verbos}. ¿Continuar? (s/n): ")
            if confirmar.lower() == 's':
                procesar_carpeta(carpeta_verbos, hacer_backup=True)
        else:
            print(f"❌ Carpeta no encontrada: {carpeta_verbos}")
